...
REMOTE_FRONTEND_DIR="$REMOTE_RELEASE_DIR/aitrade-fe-dist"
...
log '在远端解压后端源码包并切换 current 软链'
ssh "$SSH_ALIAS" "set -euo pipefail; rm -rf '$REMOTE_RELEASE_DIR'; mkdir -p '$REMOTE_RELEASE_DIR'; tar -xzf '$REMOTE_ARCHIVE_PATH' -C '$REMOTE_RELEASE_DIR'; if [ ! -f '$REMOTE_SHARED_DIR/config.yaml' ]; then cp '$REMOTE_BACKEND_DIR/config.example.yaml' '$REMOTE_SHARED_DIR/config.yaml'; fi; ln -sfn '$REMOTE_SHARED_DIR/config.yaml' '$REMOTE_BACKEND_DIR/config.yaml'; ln -sfn '$REMOTE_RELEASE_DIR' '$REMOTE_CURRENT_LINK'"

log '上传前端静态产物'
ssh "$SSH_ALIAS" "rm -rf '$REMOTE_FRONTEND_DIR' && mkdir -p '$REMOTE_FRONTEND_DIR'"
scp -r "$FRONTEND_DIST_DIR/." "$SSH_ALIAS:$REMOTE_FRONTEND_DIR/"
...
printf 'current 软链: %s\n' "$REMOTE_CURRENT_LINK"
printf '\n后续建议命令：\n'
printf 'ssh %s "cd %s && bash init-env.sh"\n' "$SSH_ALIAS" "$REMOTE_CURRENT_LINK/aitrade-be"
printf 'ssh %s "cd %s && bash start-web.sh"\n' "$SSH_ALIAS" "$REMOTE_CURRENT_LINK/aitrade-be"
